...
MarkdownSetup.init = () ->
  if MarkdownSetup.rendererRequired()
    MarkdownSetup.converter = new Markdown.Converter();
...
$ ->
  ...
  previewCntr = $('#preview-container')
  inputField = $('#new-comment')
  markdownHelpBtn = $('#markdown-help')
  markdownHelpBtn.click ->
    $('#wmd-help').modal('toggle')
  previewBtn.click ->
    previewBtn.toggleClass('active')
    inputField.toggle()
    previewCntr.html(inputField.val())
    MarkdownSetup.render_markdown('#comment-input')
    previewCntr.toggle()
...
MarkdownSetup.render_markdown = (scope = 'body') ->
  ...
  $(scope + ' .js-render-as-markdown').each ->
        # 5. replace the content with HTML
